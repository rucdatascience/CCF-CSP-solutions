...
令f[1,i]为在障碍物(a1,ai)的区间内种树的方案数，有DP递归表达式：f[1,i]=sum_{1<=j<i}f[1,j]*f[j,i]，其中f[1,1]=1.
最终解：f[1,n]/MOD
...
如果该约数被之前枚举的j用到过，则使用该约数会在之前枚举的j障碍处种树，故该约数不对应ai-aj区间的一种可行方案。
基于上述原理快速计算f[j,i]，即ai-aj区间的可行方案数量。
*/
#include <bits/stdc++.h>
using namespace std;
